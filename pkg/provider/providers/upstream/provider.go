...
func (p *upstreamProvider) metricsFor(
	...
	for _, metricValue := range metricValueList.Items {
		...
		metric, err := p.metricFor(ctx, timestampedMetric, namespacedName, info, metricSelector)
		if err != nil {
			return nil, err
		}
...
func (p *upstreamProvider) GetMetricByName(name types.NamespacedName, info provider.CustomMetricInfo, metricSelector labels.Selector) (*custom_metrics.MetricValue, error) {
	tx, ctx := tracing.NewTransaction(context.TODO(), p.tracer, "upstream-provider", "GetMetricByName")
	defer tracing.Span(&ctx)()
	...
	if info.Namespaced {
		...
		return p.metricFor(&ctx, common.TimestampedMetric{
			Value:     object.Value,
...
func (p *upstreamProvider) GetMetricBySelector(namespace string, selector labels.Selector, info provider.CustomMetricInfo, metricSelector labels.Selector) (*custom_metrics.MetricValueList, error) {
	tx, ctx := tracing.NewTransaction(context.TODO(), p.tracer, "upstream-provider", "GetMetricBySelector")
	defer tracing.Span(&ctx)()
	defer tracing.EndTransaction(tx)
	...
	if info.Namespaced {
		...
		return p.metricsFor(&ctx, info, metricSelector, objects)
	}
