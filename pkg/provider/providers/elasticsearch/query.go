...
func getMetricForPod(
	...
	if res.IsError() {
		...
		if err := json.NewDecoder(res.Body).Decode(&e); err != nil {
			return common.TimestampedMetric{}, fmt.Errorf("error parsing the response body: %s", err)
		} else {
			// Print the response status and error information.
			return common.TimestampedMetric{}, fmt.Errorf("[%s] %s: %s",
				res.Status(),
	...
	return common.TimestampedMetric{
		Value:     *q,
		Timestamp: timestamp,
	}, nil
}
